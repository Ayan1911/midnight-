...
export class Contract {
      ...
      castVote: (...args_1) => {
        ...
        if (!(typeof(contextOrig_0) === 'object' && contextOrig_0.currentQueryContext != undefined)) {
          __compactRuntime.typeError('castVote',
                                     'argument 1 (as invoked from Typescript)',
                                     'voting.compact line 40 char 1',
                                     'CircuitContext',
                                     contextOrig_0)
        }
        if (!(typeof(candidate_0) === 'bigint' && candidate_0 >= 0n && candidate_0 <= 255n)) {
          __compactRuntime.typeError('castVote',
        ...
        const result_0 = this._castVote_0(context, partialProofData, candidate_0);
        partialProofData.output = { value: [], alignment: [] };
        return { result: result_0, context: context, proofData: partialProofData, gasCost: context.gasCost };
      }
    };
  ...
  _castVote_0(context, partialProofData, candidate_0) {
    ...
    __compactRuntime.assert(this._equal_0(candidate_0, 0n)
                            ||
                            this._equal_1(candidate_0, 1n),
                            'Invalid candidate: must be 0 (Candidate A) or 1 (Candidate B)');
    const voterSecret_0 = this._getVoterSecret_0(context, partialProofData);
    const nullifier_0 = this._persistentHash_0(voterSecret_0);
    __compactRuntime.assert(!_descriptor_0.fromValue(__compactRuntime.queryLedgerState(context,
                                                                                       partialProofData,
                                                                                       [
                                                                                        { dup: { n: 0 } },
                                                                                        ...
                                                                                        { popeq: { cached: true,
                                                                                                   result: undefined } }]).value),
                            'Double-vote rejected: voter nullifier has already been spent');
    __compactRuntime.queryLedgerState(context,
                                      partialProofData,
                                      [
                                       { idx: { cached: false,
    ...
    __compactRuntime.queryLedgerState(context,
                                      ...
                                      [
                                       ...
                                       { ins: { cached: true, n: 1 } }]);
    if (this._equal_2(candidate_0, 0n)) {
      const tmp_1 = 1n;
      __compactRuntime.queryLedgerState(context,
                                        partialProofData,
                                        [
...
const _dummyContract = new Contract({ getVoterSecret: (...args) => undefined });
